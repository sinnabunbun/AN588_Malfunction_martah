...
# Amazing notes! Looks like you are on top of this!
...
```
# These look like good values to test the function with 
## Regression
...
# good basic lm(), to see a more detailed summary with p values and r squared stats use summary(m)
# to summarize the object you made
...
# Looks good with the geom_smooth argument but I think he would like us to create the confidence intervals ourselves 

# To do that use the predict() function on the linear model you created and use the argument interval = "condifence". This will give you a vector with the y values of the best fit line, the upper condifidnce interval and lower confidence interval. You can put these on the graph by creating lines for each using geom_line. 

# the confint() function can be used to find the confidence intervals of the slope and the intercept

# Also the predict() function can be used to find a specific value of a predction like 800 by using interval = "prediction"
...
# These are not the results which I got for this graph. Try the by hand method for the linear model line and you should see that the graph is curved. 
